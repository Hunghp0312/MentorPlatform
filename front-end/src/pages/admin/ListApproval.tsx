...
import useDebounce from "../../hooks/usedebounce";
import { ApprovalType } from "../../types/approval";
import { mockApprovals } from "../../data/_mockapproval";
import { mockDocuments } from "../../data/_mockmentorapplicationdata";

// Define a temporary interface to match the UI display requirements
interface DisplayApprovalType extends ApprovalType {
  name: string;
  email: string;
...
// Map mockApprovals and mockDocuments to the display format
const mapToDisplayApprovals = (
  ...
  return approvals.map((approval) => {
    // Derive name, email, expertiseAreas, and experience from available data
    const name = `Mentor ${approval.id}`; // Placeholder since name is not in ApprovalType
    const email = `mentor${approval.id}@example.com`; // Placeholder
    const expertiseAreas = approval.motivationStatement
      .split(" ")
      .slice(0, 3)
      .map((word) => word.charAt(0).toUpperCase() + word.slice(1)); // Derive from motivationStatement
    const experience = approval.motivationStatement.slice(0, 50) + "..."; // Truncate motivationStatement
    ...
    // Map documents for this approval
    const approvalDocs = documents
      .filter((doc) => doc.mentorApplication.id === approval.id)
      .map((doc) => ({
        type: doc.fileType.split("/")[1].toUpperCase(), // e.g., "pdf" -> "PDF"
        name: doc.fileName,
        url: "#", // Placeholder URL
      }));
...
// Mock data adapted to the UI
const initialApprovals: DisplayApprovalType[] = mapToDisplayApprovals(
  mockApprovals,
  mockDocuments
);
...
const ListApproval = () => {
  const [approvals, setApprovals] =
    useState<DisplayApprovalType[]>(initialApprovals);
  const [selectedApproval, setSelectedApproval] =
    useState<DisplayApprovalType | null>(null);
  const [loading, setLoading] = useState(false);
  const [searchTerm, setSearchTerm] = useState("");
  const [statusFilter, setStatusFilter] = useState("");
  const [totalItems, setTotalItems] = useState(0);
  ...
  useEffect(() => {
    setLoading(true);
    const filtered = initialApprovals.filter((approval) => {
      const matchesSearch = approval.name
        .toLowerCase()
        .includes(searchDebounced.toLowerCase());
      const matchesStatus = statusFilter
        ? approval.status.name === statusFilter
        : true;
      return matchesSearch && matchesStatus;
    });
    const start = (pageIndex - 1) * pageSize;
  ...
  // Define columns for CustomTable
  const columns: DataColumn<DisplayApprovalType>[] = [
    {
      header: "",
      accessor: (row) => (
        <div className="flex items-center space-x-4">
  ...
  // Mock action handlers
  const handleApprove = (approval: DisplayApprovalType) => {
    if (window.confirm(`Approve application for "${approval.name}"?`)) {
      setApprovals((prev) =>
        prev.map((item) =>
          item.id === approval.id
            ? { ...item, status: { id: 2, name: "Approved" } }
            : item
        )
      );
      setTotalItems(initialApprovals.length);
      toast.success(`Application for ${approval.name} approved`);
      if (selectedApproval?.id === approval.id) {
        setSelectedApproval({
          ...approval,
  ...
  const handleReject = (approval: DisplayApprovalType) => {
    if (window.confirm(`Reject application for "${approval.name}"?`)) {
      setApprovals((prev) =>
        prev.map((item) =>
          item.id === approval.id
            ? { ...item, status: { id: 3, name: "Rejected" } }
            : item
        )
      );
      setTotalItems(initialApprovals.length);
  ...
  const handleSelectApplicants = (approval: DisplayApprovalType) => {
    setSelectedApproval(approval);
  };
                ...
                <div className="bg-gray-700 rounded-lg p-4 text-gray-300">
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm">
                        Submitted on {selectedApproval.submissionDate}
                      </p>
                    </div>
                    <div>
                      <h4 className="text-sm font-medium text-gray-400">
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        placeholder="Add notes about this application..."
                        rows={3}
                        value={selectedApproval.adminComment || ""}
                        onChange={(e) =>
                          setSelectedApproval({
                            ...selectedApproval,
