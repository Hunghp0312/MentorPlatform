...
namespace ApplicationCore.Validators.Course
{
    public class UpdateCourseRequestDtoValidator : AbstractValidator<UpdateCourseRequestDto>
    {
        public UpdateCourseRequestDtoValidator()
        {
            // Title validation
            RuleFor(x => x.Title)
                .NotEmpty().WithMessage("Please fill out this field")
                .MaximumLength(100).WithMessage("Title should be 1 - 100 characters")
                .Must(title => !string.IsNullOrWhiteSpace(title))
                .WithMessage("Please fill out this field");

            // Category validation
            RuleFor(x => x.CategoryId)
                .NotEmpty().WithMessage("Please select an item in the list")
                .NotEqual(Guid.Empty).WithMessage("Please select an item in the list");
            ...
            RuleFor(x => x.Duration)
                .NotEmpty().WithMessage("Please fill out this field")
                .MinimumLength(6).WithMessage("Duration should be 6-100 characters")
                .MaximumLength(100).WithMessage("Duration should be 6-100 characters")
                .Must(duration => !string.IsNullOrWhiteSpace(duration))
                .WithMessage("Please fill out this field");

            // Tags validation
            RuleForEach(x => x.Tags)
                .MaximumLength(50).WithMessage("Tag should be 1-50 characters")
            ...
            RuleFor(x => x.Description)
                .NotEmpty().WithMessage("Please fill out this field")
                .MaximumLength(1000).WithMessage("Description should be 1-1000 characters")
